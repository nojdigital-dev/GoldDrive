    ...
    <div className="min-h-screen bg-black text-white flex flex-col relative overflow-hidden">
       ...
       <div className="flex-1 flex flex-col justify-end sm:justify-center px-8 sm:max-w-md mx-auto w-full z-10 pb-12">
           ...
           <form onSubmit={handleAuth} className="space-y-4">
               {isSignUp && (
                   <Input placeholder="Nome Completo" className="h-14 bg-white/5 border-white/10 text-white placeholder:text-gray-500 text-lg rounded-xl focus:border-yellow-500 focus:ring-yellow-500" value={name} onChange={e => setName(e.target.value)} disabled={loading} />
               )}
               <Input type="email" placeholder="Email cadastrado" className="h-14 bg-white/5 border-white/10 text-white placeholder:text-gray-500 text-lg rounded-xl focus:border-yellow-500 focus:ring-yellow-500" value={email} onChange={e => setEmail(e.target.value)} disabled={loading} />
               <Input type="password" placeholder="Sua senha" className="h-14 bg-white/5 border-white/10 text-white placeholder:text-gray-500 text-lg rounded-xl focus:border-yellow-500 focus:ring-yellow-500" value={password} onChange={e => setPassword(e.target.value)} disabled={loading} />
           ...
           <div className="mt-8 text-center z-20">
               <button onClick={() => setIsSignUp(!isSignUp)} className="text-sm font-medium text-gray-400 hover:text-white transition-colors" disabled={loading}>{isSignUp ? "Já sou parceiro, fazer login" : "Quero ser motorista parceiro"}</button>
           </div>
       </div>
    </div>
  );
