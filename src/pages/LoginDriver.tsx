       ...
       <div className="flex-1 flex flex-col justify-end sm:justify-center px-8 sm:max-w-md mx-auto w-full z-10 pb-12">
           <div className="mb-8 animate-in slide-in-from-bottom-10 fade-in duration-700">
               ...
               <h1 className="text-4xl font-bold tracking-tight mb-2">
                   Área do <span className="text-yellow-500">Parceiro</span>
               </h1>
               <p className="text-gray-400 text-lg">
                   {isSignUp ? "Junte-se à frota GoldDrive." : "Gerencie seus ganhos e corridas."}
               </p>
           </div>
           ...
           <form onSubmit={handleAuth} className="space-y-4 animate-in slide-in-from-bottom-5 fade-in duration-1000 delay-100">
               {isSignUp && (
                   <Input 
                       ...
                       className="h-14 bg-white/5 border-white/10 text-white placeholder:text-gray-500 text-lg rounded-xl focus:border-yellow-500 focus:ring-yellow-500"
               ...
               <Button 
                   className="w-full h-14 text-lg font-bold rounded-xl bg-yellow-500 hover:bg-yellow-400 text-black mt-4"
                   disabled={loading}
               >
           ...
           <div className="mt-8 text-center z-20">
               <button onClick={() => setIsSignUp(!isSignUp)} className="text-sm font-medium text-gray-400 hover:text-white transition-colors">
                   {isSignUp ? "Já sou parceiro, fazer login" : "Quero ser motorista parceiro"}
               </button>
           </div>
       </div>
    </div>
