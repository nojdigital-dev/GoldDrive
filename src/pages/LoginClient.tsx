...
const LoginClient = () => {
  ...
  return (
    <div className="min-h-screen relative flex items-center justify-center font-sans overflow-hidden">
       {/* Global Background (Mobile & Desktop) */}
       ...
       <div className="relative z-10 w-full lg:w-full flex h-full min-h-screen lg:min-h-0">
           ...
           {/* Form Area - Full width on Mobile with Glassmorphism */}
           <div className="w-full lg:w-1/2 flex flex-col relative bg-transparent lg:bg-white">
               
               {/* Mobile: Back Button needs to be white/visible on dark background */}
               <div className="p-6 absolute top-0 left-0">
                   <Button variant="ghost" onClick={() => navigate('/')} className="hover:bg-white/10 lg:hover:bg-gray-100 rounded-full w-12 h-12 p-0 -ml-2 text-white lg:text-gray-800">
                       <ArrowLeft className="w-6 h-6" />
                   </Button>
               </div>

               <div className="flex-1 flex flex-col justify-center px-6 sm:px-16 max-w-xl mx-auto w-full">
                   
                   {/* Mobile Card Container */}
                   <div className="bg-white/95 backdrop-blur-xl lg:bg-transparent lg:backdrop-blur-none p-8 lg:p-0 rounded-[32px] lg:rounded-none shadow-2xl lg:shadow-none animate-in fade-in slide-in-from-bottom-10 duration-500">
                       ...
                       <form onSubmit={handleAuth} className="space-y-5">
                           {isSignUp && (
                               <div className="relative group">
                                   ...
                                   <Input placeholder="Nome Completo" className="h-14 pl-12 bg-gray-50 border-gray-200 text-base rounded-2xl focus:ring-2 focus:ring-yellow-500/20 focus:border-yellow-500 transition-all text-slate-900" value={name} onChange={e => setName(e.target.value)} disabled={loading} />
                               </div>
                           )}
                           <div className="relative group">
                               <Mail className="absolute left-4 top-4 w-5 h-5 text-gray-400 group-focus-within:text-yellow-600 transition-colors" />
                               <Input type="email" placeholder="Endereço de Email" className="h-14 pl-12 bg-gray-50 border-gray-200 text-base rounded-2xl focus:ring-2 focus:ring-yellow-500/20 focus:border-yellow-500 transition-all text-slate-900" value={email} onChange={e => setEmail(e.target.value)} disabled={loading} />
                           </div>
                           <div className="relative group">
                               ...
                               <Input 
                                    ...
                                    className="h-14 pl-12 pr-10 bg-gray-50 border-gray-200 text-base rounded-2xl focus:ring-2 focus:ring-yellow-500/20 focus:border-yellow-500 transition-all text-slate-900" 
                                    value={password} 
                           ...
                           <Button className="w-full h-14 text-lg font-bold rounded-2xl bg-slate-900 hover:bg-black text-white shadow-xl shadow-slate-900/10 transition-all hover:scale-[1.02] active:scale-[0.98]" disabled={loading}>
