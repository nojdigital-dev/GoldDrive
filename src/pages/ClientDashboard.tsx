...
const ClientDashboard = () => {
  ...
  useEffect(() => {
    if (ride) {
      ...
      setShowArrivalPopup(false); setShowStartPopup(false);
    }
  }, [ride]);
  ...
  const fetchInitialData = async () => {
    ...
    } catch (error) { console.error("Erro fetch:", error); } finally { setLoadingCats(false); }
  };

  const handleTabChange = (tab: string) => { if (tab === 'profile') navigate('/profile'); else if (tab === 'wallet') navigate('/wallet'); else setActiveTab(tab); };
  ...
  const confirmRide = async () => {
    ...
    if (paymentMethod === 'WALLET' && (userProfile?.balance || 0) < price) { setMissingAmount(price - (userProfile?.balance || 0)); setShowBalanceAlert(true); return; }
    setIsRequesting(true);
    try { await requestRide(pickup, dest.label, price, dest.distance, cat.name, paymentMethod); } catch (e: any) { showError(e.message); } finally { setIsRequesting(false); }
  };

  const getCurrentLocation = () => { setLoadingLocation(true); navigator.geolocation.getCurrentPosition(() => { setPickup(`Rua das Flores, 123`); setLoadingLocation(false); }, () => { showError("Erro GPS"); setLoadingLocation(false); }); };
  ...
  return (
    <div className="relative h-screen w-full overflow-hidden font-sans bg-gray-100">
      <div className="absolute inset-0 z-0"><MapComponent showPickup={step !== 'search'} showDestination={!!destinationId && step !== 'search'} /></div>
      ...
      <div className="absolute top-0 left-0 right-0 p-6 z-20 flex justify-between items-start pointer-events-none">
          ...
          <div className="pointer-events-auto bg-black text-white px-4 py-2.5 rounded-full flex items-center gap-2 shadow-xl animate-in slide-in-from-top duration-500 delay-100 cursor-pointer hover:bg-zinc-800 transition-colors" onClick={() => navigate('/wallet')}>
              <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse" /><span className="font-bold text-sm tracking-tight">R$ {userProfile?.balance?.toFixed(2) || '0.00'}</span>
          </div>
      </div>
      ...
      <div className="absolute inset-0 z-10 flex flex-col justify-end pb-32 md:pb-10 md:justify-center items-center pointer-events-none p-4">
        {activeTab === 'home' && (
            <div className="w-full max-w-md pointer-events-auto transition-all duration-500">
                {step === 'search' && (
                    <div className={cardBaseClasses}>
                        ...
                        <div className="space-y-4">
                            <div className="relative group"><div className="absolute left-4 top-4 w-3 h-3 rounded-full bg-blue-500 ring-4 ring-blue-500/20 z-10"></div><Input value={pickup} onChange={(e) => setPickup(e.target.value)} placeholder="Sua localização" className="pl-12 h-14 bg-gray-50/50 border-transparent hover:bg-white focus:bg-white rounded-2xl transition-all shadow-sm" /><Button size="icon" variant="ghost" className="absolute right-2 top-2 text-blue-600 hover:bg-blue-50 rounded-xl" onClick={getCurrentLocation} disabled={loadingLocation}><Navigation className={`w-5 h-5 ${loadingLocation ? 'animate-spin' : ''}`} /></Button></div>
                            <div className="relative group"><div className="absolute left-[19px] -top-6 w-0.5 h-8 bg-gray-300 z-0"></div><div className="absolute left-4 top-4.5 w-3 h-3 bg-black ring-4 ring-black/10 z-10"></div><Select onValueChange={setDestinationId} value={destinationId}><SelectTrigger className="pl-12 h-14 bg-gray-50/50 border-transparent hover:bg-white rounded-2xl transition-all shadow-sm text-base font-medium"><SelectValue placeholder="Selecione o destino" /></SelectTrigger><SelectContent>{MOCK_LOCATIONS.map(loc => (<SelectItem key={loc.id} value={loc.id}>{loc.label}</SelectItem>))}</SelectContent></Select></div>
                        </div>
                ...
                {step === 'confirm' && (
                    <div className={cardBaseClasses}>
                        <div className="flex items-center gap-3 mb-6 cursor-pointer" onClick={() => setStep('search')}><div className="p-2 bg-gray-100 rounded-full hover:bg-gray-200"><ArrowLeft className="w-5 h-5" /></div><h2 className="text-xl font-bold">Escolha a Categoria</h2></div>
                        ...
                        <div className="mb-4 bg-gray-50 p-3 rounded-2xl border border-gray-100 flex items-center justify-between cursor-pointer hover:bg-white active:scale-95 transition-all" onClick={() => setShowPaymentSheet(true)}>
                             <div className="flex items-center gap-3"><div className="w-10 h-10 bg-black text-white rounded-xl flex items-center justify-center">{paymentMethod === 'WALLET' ? <Wallet className="w-5 h-5" /> : <Banknote className="w-5 h-5" />}</div><div><p className="text-xs text-gray-400 font-bold uppercase">Pagamento</p><p className="font-bold text-slate-900">{paymentMethod === 'WALLET' ? 'Saldo da Carteira' : 'Na hora (Dinheiro/PIX)'}</p></div></div><ChevronRight className="w-5 h-5 text-gray-400" />
                        </div>
                     ...
                     <div className={`${cardBaseClasses} text-center`}>
                         {ride?.status === 'ACCEPTED' || ride?.status === 'IN_PROGRESS' || ride?.status === 'ARRIVED' ? (
                            <div className="space-y-6">
                                ...
                                <div className="bg-gray-100 hover:bg-gray-200 p-3 rounded-2xl flex items-center gap-3 cursor-pointer transition-colors" onClick={() => setShowChat(true)}>
                                    <div className="w-10 h-10 bg-white rounded-full flex items-center justify-center text-slate-900 shadow-sm"><MessageCircle className="w-5 h-5" /></div>
                                    <div className="flex-1 text-left"><p className="text-xs font-bold text-gray-500 uppercase">Mensagem para motorista</p><p className="text-sm font-medium text-slate-900">Enviar mensagem...</p></div>
                                </div>
                ...
                {step === 'rating' && (
                     <div className={`${cardBaseClasses} text-center`}>
                         <div className="w-20 h-20 bg-green-100 rounded-full mx-auto flex items-center justify-center mb-6"><User className="w-10 h-10 text-green-600" /></div><h2 className="text-2xl font-black text-slate-900 mb-2">Chegamos!</h2><p className="text-gray-500 mb-6">Como foi sua experiência?</p>
                         ...
                         <div className="mb-6 text-left"><label className="text-xs font-bold text-gray-500 uppercase ml-1">Observação (Opcional)</label><Textarea placeholder="O carro estava limpo? O motorista foi educado?" className="bg-gray-50 border-gray-100 rounded-2xl resize-none mt-1" value={ratingComment} onChange={e => setRatingComment(e.target.value)} /></div>
                    ...
                    historyItems.map(item => (
                        <div key={item.id} onClick={() => setSelectedHistoryItem(item)} className="mb-3 p-4 bg-white/50 border border-white/60 rounded-2xl hover:bg-white hover:scale-[1.02] transition-all cursor-pointer shadow-sm">
                            <div className="flex justify-between mb-1">
                                <span className="font-bold text-sm text-slate-900">{new Date(item.created_at).toLocaleDateString()} <span className="text-gray-400 font-normal text-xs">• {new Date(item.created_at).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span></span>
                            ...
                            <div className="flex items-center gap-3"><div className="w-10 h-10 bg-gray-100 rounded-full flex items-center justify-center shrink-0"><MapPin className="w-5 h-5 text-gray-500" /></div><div className="flex-1 min-w-0"><p className="font-medium truncate text-sm">{item.destination_address}</p><p className="text-xs text-gray-500">R$ {Number(item.price).toFixed(2)}</p></div></div>
                        </div>
                    ))}
                </ScrollArea>
            </div>
      ...
      <Sheet open={showPaymentSheet} onOpenChange={setShowPaymentSheet}>
          <SheetContent side="bottom" className="rounded-t-[32px] p-0 overflow-hidden pb-8">
              <SheetHeader className="p-6 pb-2"><SheetTitle>Forma de Pagamento</SheetTitle></SheetHeader>
              <div className="p-6 space-y-3">
                  <div onClick={() => { setPaymentMethod('WALLET'); setShowPaymentSheet(false); }} className={`flex items-center gap-4 p-4 rounded-2xl border cursor-pointer transition-all ${paymentMethod === 'WALLET' ? 'border-yellow-500 bg-yellow-50' : 'border-gray-100 bg-white hover:bg-gray-50'}`}>
                      <div className="w-12 h-12 bg-black text-white rounded-full flex items-center justify-center"><Wallet className="w-6 h-6" /></div>
                      <div><p className="font-bold text-slate-900">Saldo da Carteira</p><p className="text-sm text-gray-500">Desconto automático</p></div>
                  </div>
                  <div onClick={() => { setPaymentMethod('CASH'); setShowPaymentSheet(false); }} className={`flex items-center gap-4 p-4 rounded-2xl border cursor-pointer transition-all ${paymentMethod === 'CASH' ? 'border-yellow-500 bg-yellow-50' : 'border-gray-100 bg-white hover:bg-gray-50'}`}>
                      <div className="w-12 h-12 bg-green-600 text-white rounded-full flex items-center justify-center"><Banknote className="w-6 h-6" /></div>
                      <div><p className="font-bold text-slate-900">Pagar na Hora</p><p className="text-sm text-gray-500">Dinheiro ou PIX direto pro motorista</p></div>
                  </div>
              </div>
          </SheetContent>
      </Sheet>

      <Dialog open={showArrivalPopup} onOpenChange={setShowArrivalPopup}><DialogContent className="sm:max-w-md bg-white border-0 shadow-2xl rounded-[40px] p-0 overflow-hidden"><div className="bg-yellow-400 h-32 relative flex items-center justify-center"><div className="absolute inset-0 bg-black/5 pattern-dots" /><div className="w-20 h-20 bg-white rounded-full flex items-center justify-center shadow-lg animate-bounce"><BellRing className="w-10 h-10 text-black fill-black" /></div></div><div className="px-8 pb-8 pt-4 text-center"><h2 className="text-3xl font-black text-slate-900 mb-2">Motorista Chegou!</h2><p className="text-gray-500 mb-6 text-lg">Seu motorista está aguardando.</p><div className="bg-gray-50 p-4 rounded-3xl flex items-center gap-4 mb-6 text-left border border-gray-100"><Avatar className="w-14 h-14 border-2 border-white shadow-sm"><AvatarImage src={ride?.driver_details?.avatar_url} /><AvatarFallback>M</AvatarFallback></Avatar><div><p className="font-bold text-lg text-slate-900">{ride?.driver_details?.name}</p><p className="text-sm text-gray-500">{ride?.driver_details?.car_model} • {ride?.driver_details?.car_plate}</p></div></div><Button className="w-full h-14 rounded-2xl text-lg font-bold bg-black hover:bg-zinc-800" onClick={() => setShowArrivalPopup(false)}>Estou indo!</Button></div></DialogContent></Dialog>
      <Dialog open={showStartPopup} onOpenChange={setShowStartPopup}><DialogContent className="sm:max-w-md bg-white border-0 shadow-2xl rounded-[40px] p-8 text-center"><div className="w-20 h-20 bg-green-100 text-green-600 rounded-full flex items-center justify-center mx-auto mb-4 animate-in zoom-in"><Flag className="w-10 h-10" /></div><h2 className="text-2xl font-black text-slate-900 mb-2">Corrida Iniciada</h2><p className="text-gray-500">Aproveite sua viagem.</p></DialogContent></Dialog>
